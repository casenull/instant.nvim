...
wss.on("request", (req) => {
    ...
    if (msg.type == "utf8") {
      ...
      if (decoded !== undefined) {
        ...
        if (decoded[0] == MSG_INFO) {
          ...
          const response = [MSG_AVAILABLE, is_first, client_id, sessionshare];

          for (var v of clientUsernames) {
            const connect = [MSG_CONNECT, v[0], v[1]];
            ws.sendUTF(JSON.stringify(connect));
          }

          const connect = [MSG_CONNECT, client_id, decoded[2]];
  ...
  ws.on("close", (reasonCode, desc) => {
    ...
    const disconnect = [MSG_DISCONNECT, remove_id];
